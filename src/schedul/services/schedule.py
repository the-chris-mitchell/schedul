...
def formatted(screening: Screening) -> str:
    return f"{start_time_formatted(screening)}-{end_time_formatted(screening)}: {screening.film.name} ({screening.film.runtime} minutes) @ {screening.venue.name} {'✅' if screening.id in CONFIG.booked_sessions else screening.id}"
